...
\title{Estimadores de Máxima Verossimilhança para Assimetria e Curtose
usando a expansão de Gram-Charlier}
...
volatilidade no mercado. A Figure~\ref{fig-skewness-kurtosis-ma}
...
\includegraphics{estimadores-assimetria-curtose_files/figure-pdf/fig-skewness-kurtosis-ma-1.pdf}
...
Nos gráficos de média móvel de assimetria e curtose, na
Figure~\ref{fig-skewness-kurtosis-ma}, é possível observar que em
diversos momentos há mudanças de nível significativas, relacionadas a
períodos de maior volatilidade nos retornos. Entretanto, o gráfico com a
média móvel do desvio padrão, apresenta um comportamento mais suave, nos
mesmos períodos.
...
\section{Expansão de Gram-Charlier}\label{expansuxe3o-de-gram-charlier}
...
\(p(x)\)}{Restrições de positividade para p(x)}}\label{restriuxe7uxf5es-de-positividade-para-px}
...
\includegraphics{estimadores-assimetria-curtose_files/figure-pdf/fig-random-heatmap-1.pdf}

}

\caption{\label{fig-random-heatmap}O gráfico acima é um \emph{heatmap}
da função \(l(\mu_3, \mu_4; \mathbf{x})\) para uma amostra aleatória
normal. Os valores da função de log-verossimilhança são segmentados de
forma que é possível identificar que a região em torno dos momentos
populacionais, \(\mu_3=0\) e \(\mu_4 = 3\), apresenta a maior
verossimilhança. O ponto marcado com X indica os valores amostrais de
assimetria e curtose. Os pontos escuros onde a função de
...
Observando a Figure~\ref{fig-random-heatmap} fica claro que há uma
...
A título de ilustração a Figure~\ref{fig-ibovespa-heatmap} mostra o
...
\includegraphics{estimadores-assimetria-curtose_files/figure-pdf/fig-ibovespa-heatmap-1.pdf}

}

\caption{\label{fig-ibovespa-heatmap}O gráfico mostra o \emph{heatmap}
da função de log-verossimilhança aplicada a série de retornos
padrinizados do IBOVESPA no período de jan/2000 a dez/2017. Nota-se que
a região de máxima verossimilhança desloca-se para onde a assimetria é
negativa e a curtose é um pouco acima de 4, indicando uma distribuição
assimétrica e com caudas pesadas. Entretanto, os estimadores amostrais
ficam próximos a borda da região válida, muito distantes da região de
...
condicionais. Como observa-se nas Figure~\ref{fig-random-heatmap} e
Figure~\ref{fig-ibovespa-heatmap} há um plano de coordenadas
\((\mu_3, \mu_4)\) onde a região definida por uma figura oval nos
gráficos é a região de validade para os parâmetros \(\mu_3\) e
\(\mu_4\). Vamos começar determinando a borda dessa região.
...
este sistema determinam a borda da região oval observada nas
Figure~\ref{fig-random-heatmap} e Figure~\ref{fig-ibovespa-heatmap}. A
...
Table~\ref{tbl-regions} apresenta as regiões de \(x\) que atendem as
restrições para as equações de \(\mu_3(x)\) e \(\mu_4(x)\), sabendo que
\(\mu_3(x) \geq 0\) ou \(\mu_3(x) < 0\) e que \(\mu_4(x) \geq 3\).

\begin{longtable}[]{@{}ll@{}}
\caption{Regiões de \(x\) que atendem as restrições de \(\mu_3(x)\) e
\(\mu_4(x)\).}\label{tbl-regions}\tabularnewline
\toprule\noalign{}
...
\includegraphics{estimadores-assimetria-curtose_files/figure-pdf/fig-eggs-1.pdf}
...
Como observa-se na Figure~\ref{fig-eggs}, a busca de parâmetros
...
\(\mu_4^\prime\). A Figure~\ref{fig-limit-transform} mostra o gráfico da
...
\includegraphics{estimadores-assimetria-curtose_files/figure-pdf/fig-limit-transform-1.pdf}
...
Gram-Charlier}\label{estimauxe7uxe3o-da-assimetria-e-curtose-com-a-expansuxe3o-de-gram-charlier}
...
A Table~\ref{tbl-mv-results} apresenta uma comparação entre os
...
Figure~\ref{fig-gc-dist} apresenta as funções de densidade geradas com
...
tempo. Na Figure~\ref{fig-skewness-kurtosis-ma} observamos o
comportamento dos estimadores amostrais ao longo do tempo em uma janela
móvel. A Figure~\ref{fig-mle-skewness-kurtosis-ma} reproduz os
resultados da Figure~\ref{fig-skewness-kurtosis-ma} utilizando os
...
\caption{\label{tbl-mv-results}Resultados do cálculo dos estimadores de
assimetria e curtose: máxima-verossimilhança e amostral, para a amostra
aleatória normal e para a série de retornos padronizados do IBOVESPA.}
...
\midrule
Assimetria & 0.0000274 & 0.091725 & -0.1279683 & -0.111351\\
Curtose & 3.0000166 & 2.847665 & 4.2655168 & 6.857522\\
\bottomrule
...
\includegraphics{estimadores-assimetria-curtose_files/figure-pdf/fig-gc-dist-1.pdf}
...
\includegraphics{estimadores-assimetria-curtose_files/figure-pdf/fig-mle-skewness-kurtosis-ma-1.pdf}
...
séries. Por exemplo, na Figure~\ref{fig-mle-skewness-kurtosis-ma}
...
\bibitem[\citeproctext]{ref-blinnikov1998}
\CSLLeftMargin{{[}1{]} }%
\CSLRightInline{S. Blinnikov and R. Moessner, Astronomy and Astrophysics
